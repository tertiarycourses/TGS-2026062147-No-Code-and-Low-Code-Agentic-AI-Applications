#!/bin/bash
# MediRefill — one-click launcher (macOS)
# Double-click in Finder (first time: right-click → Open to pass Gatekeeper).
# Serves the site on a local port (so the browser trusts it for the microphone)
# and opens it. Uses Python if available, else Node (npx).
cd "$(dirname "$0")" || exit 1
